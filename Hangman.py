...
words_list = [ #создаем список из русских слов
    ...
    "интернет", "реальность", "робототехника", "интеллект", "блокчейн",
...
flag = False #объявляется булевая переменная - флаг для того, чтобы понять, сдался ли игрок
while "*" in secret_word_list: #объвляется цикл while, который выполняется, пока в списке из загаданного слова остаются символы *
    ...
    if smth in commands: #происходит проверка: если игрок ввел команду, то программа совершает действие команды
        ...
        if smth == "/surrender": #происходит проверка: если игрок подал команду сдаться, цикл прерывается
            flag = commands[smth]
            break
        print(commands[smth])
...
if flag:
    print("Вы сдались! Не расстраивайтесь! Повезет в следующий раз!") #игрок сдался, выводится соответствующее сообщение
